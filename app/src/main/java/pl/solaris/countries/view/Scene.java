package pl.solaris.countries.view;
...
//                    }
    ...
    /**
     * <p>The CacheThread's job is to wait until the {@link pl.solaris.countries.view.Scene.Cache#state} is
     * {@link pl.solaris.countries.view.Scene.CacheState#START_UPDATE} and then update the {@link pl.solaris.countries.view.Scene.Cache} given
     * the current {@link pl.solaris.countries.view.Scene.Viewport#window}. It does not want to hold the cache
     * lock during the call to {@link pl.solaris.countries.view.Scene#fillCache(android.graphics.Rect)} because the call
     * can take a long time. If we hold the lock, the user experience is very 
     * jumpy.</p>
     * <p>The CacheThread and the {@link pl.solaris.countries.view.Scene.Cache} work hand in hand, both using the
     * cache itself to synchronize on and using the {@link pl.solaris.countries.view.Scene.Cache#state}.
     * The {@link pl.solaris.countries.view.Scene.Cache} is free to update any part of the cache object as long
     * as it holds the lock. The CacheThread is careful to make sure that it is
     * the {@link pl.solaris.countries.view.Scene.Cache#state} is {@link pl.solaris.countries.view.Scene.CacheState#IN_UPDATE} as it updates
     * the {@link pl.solaris.countries.view.Scene.Cache}. It locks and unlocks the cache all along the way, but
     * makes sure that the cache is not locked when it calls 
     * {@link pl.solaris.countries.view.Scene#fillCache(android.graphics.Rect)}.
     */
